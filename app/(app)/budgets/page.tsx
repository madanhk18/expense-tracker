...
export default async function BudgetsPage() {
  ...
  const [overallBudget, categoryBudgets, analytics, categories] =
    await Promise.all([
  ...
  const spentByCategoryId = new Map(
    analytics.categoryBreakdown.map((c) => [c.categoryId, c.paise]),
  );
  const budgetedCategoryIds = new Set(
    categoryBudgets.map((b) => b.category_id),
  );
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-4">
      <PageHeader
        title="Budgets"
        description="Set a limit and track how close you are."
      />
        ...
        <Panel>
          ...
          <CardContent>
            {overallBudget ? (
              <BudgetProgress
                label=""
                spentPaise={analytics.totalPaise}
              ...
              <EmptyState
                ...
                description="Set a budget to track your spending against it."
              />
            )}
          </CardContent>
        </Panel>
