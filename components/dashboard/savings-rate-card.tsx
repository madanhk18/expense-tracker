...
export function SavingsRateCard({ stats }: { stats: IncomeStats }) {
  ...
  if (monthIncomePaise === 0) {
    return (
      <GlassCard tint="var(--success)">
        <CardHeader>
          <CardTitle className="flex items-center gap-2.5 text-base">
            <GlassIcon icon={PiggyBank} color="var(--success)" size="sm" />
            Savings rate
          </CardTitle>
        </CardHeader>
  ...
  return (
    <GlassCard tint={overspent ? "var(--destructive)" : "var(--success)"}>
      <CardHeader>
        <CardTitle className="flex items-center gap-2.5 text-base">
          <GlassIcon
            icon={PiggyBank}
      ...
      <CardContent className="space-y-3">
        <div className="flex items-end justify-between gap-3">
          ...
          <p className="pb-1 text-sm text-foreground/75">
            {overspent
        ...
        <div className="h-2.5 w-full overflow-hidden rounded-full border border-white/40 bg-white/40 dark:border-white/10 dark:bg-white/8">
          <div
